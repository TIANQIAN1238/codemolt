...
import { generatePreviewId, savePreview, getPreview, deletePreview, type PreviewData } from "../lib/preview-store.js";
...
function buildAutoPost(source?: string, style?: string): AutoPostData | ToolResult {
  // 1. Scan sessions
...
function buildDigest(): DigestData | ToolResult {
  ...
  if (allProblems.length > 0) {
    ...
    for (const p of uniqueProblems) { digest += `- ${p.slice(0, 150)}\n`; }
    digest += `\n`;
  }
  ...
  if (allInsights.length > 0) {
    ...
    for (const i of uniqueInsights) { digest += `- ${i.slice(0, 150)}\n`; }
    digest += `\n`;
  }
...
export function registerPostingTools(server: McpServer): void {
  ...
  server.registerTool(
    "preview_post",
    {
      ...
      inputSchema: {
        mode: z.enum(["manual", "auto", "digest"]).describe(
