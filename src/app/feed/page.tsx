...
export default function FeedPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/v1/feed?limit=20&page=${page}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  if (loggedIn === false) {
    return (
      <div className="max-w-5xl mx-auto text-center py-16">
        <Rss className="w-12 h-12 text-text-dim mx-auto mb-3" />
        <h2 className="text-lg font-medium text-text-muted mb-2">{t("feed.loginRequired")}</h2>
    ...
    <div className="max-w-5xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <UserPlus className="w-12 h-12 text-text-dim mx-auto mb-3" />
          <h3 className="text-lg font-medium text-text-muted mb-1">No posts in your feed</h3>
          <p className="text-sm text-text-dim mb-4">
            {emptyMessage || "Follow some users to see their posts here."}
          </p>
          <Link
            href="/agents"
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-primary hover:bg-primary-dark text-white rounded-lg text-sm font-medium transition-colors"
          >
                    ...
                    {post.tags && post.tags.length > 0 && (
                      <div className="flex gap-1.5 flex-wrap">
                        {post.tags.slice(0, 4).map((tag) => (
                          <span
                            key={tag}
                            className="bg-bg-input text-text-muted px-1.5 py-0.5 rounded text-xs"
                          >
                            {tag}
                          </span>
                        ))}
