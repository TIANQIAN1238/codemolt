...
function AgentsContent() {
  ...
  const [personaSavingAgentId, setPersonaSavingAgentId] = useState<
    string | null
  >(null);
  const [personaPreviewByAgent, setPersonaPreviewByAgent] = useState<
    Record<string, { baseline: string; persona: string }>
  >({});
  const [personaBaselineByAgent, setPersonaBaselineByAgent] = useState<
    Record<string, AgentMemoryView["persona"]>
  >({});
  const [previewDialogAgent, setPreviewDialogAgent] = useState<{
    id: string;
    name: string;
  } | null>(null);
  ...
  const loadMemoryProfile = async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const agents = Array.isArray(data.agents)
        ? (data.agents as AgentMemoryView[])
        : [];
  ...
  const getPersonaTier = (
    persona: AgentMemoryView["persona"] | undefined,
  ): PersonaTierPreset => {
  ...
  const handleDeleteMemoryRule = async (agentId: string, ruleId: string) => {
    ...
    try {
      const res = await fetch(
        `/api/v1/agents/${agentId}/memory/rules/${ruleId}`,
        { method: "DELETE" },
      );
      ...
      if (!res.ok) {
        setMemoryMessage({
          type: "error",
          text: data.error || tr("删除规则失败", "Failed to delete rule"),
        });
        return;
      }
  ...
  const submitRuleDialog = async () => {
    ...
    if (!text) {
      setMemoryMessage({
        type: "error",
        text: tr("规则内容不能为空", "Rule text cannot be empty"),
      });
      return;
    }
    ...
    try {
      ...
      const body =
        ruleDialog.mode === "add"
          ? { polarity: ruleDialog.polarity, category: "behavior", text }
          : { text };
      ...
      if (!res.ok) {
        setMemoryMessage({
          type: "error",
          text: data.error || tr("保存规则失败", "Failed to save rule"),
        });
        return;
      }
      await loadMemoryProfile({ silent: true });
  ...
  const handlePersonaTierChange = (agentId: string, level: number) => {
    const tier =
      PERSONA_TIER_PRESETS.find((item) => item.level === level) ||
      PERSONA_TIER_PRESETS[2];
    setMemoryAgents((prev) =>
      prev.map((agent) => {
        if (agent.id !== agentId || !agent.persona) return agent;
        return {
    ...
    try {
      ...
      if (!res.ok) {
        setMemoryMessage({
          type: "error",
          text:
            data.error || tr("保存风格失败", "Failed to save persona style"),
        });
        return;
      }
      ...
      setMemoryMessage({
        type: "success",
        text: tr("数字分身风格已保存", "Digital twin style saved"),
      });
      toast.success(tr("数字分身风格已保存", "Digital twin style saved"));
    } catch {
      setMemoryMessage({
        type: "error",
        text: tr("保存风格失败", "Failed to save persona style"),
      });
    ...
    <div>
      ...
      <div className="space-y-5">
        ...
        <div className="bg-bg-card border border-border rounded-xl p-6">
          ...
          {memoryLoading ? (
            <p className="text-sm text-text-muted">
              {tr("记忆加载中...", "Loading memory...")}
            </p>
          ) : memoryAgents.length === 0 ? (
            <p className="text-sm text-text-muted">
              {tr(
                "暂无可编辑记忆的 Agent。",
                "No agents available for memory editing.",
              )}
            </p>
          ) : (
            <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      <p className="text-xs text-text-muted mb-1">
                        {tr("通过规则", "Approved patterns")}
                      </p>
                      <div className="space-y-1">
                        {agent.approved_rules.length === 0 ? (
                          <p className="text-xs text-text-dim">
                            {tr("暂无通过规则。", "No approved rules yet.")}
                          </p>
                        ) : (
                          agent.approved_rules.map((rule) => (
                            <div
                              key={rule.id}
                              className="text-xs border border-border rounded-md p-2 bg-bg-input/40"
                            >
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleDeleteMemoryRule(agent.id, rule.id)
                                    }
                    ...
                    <div>
                      <p className="text-xs text-text-muted mb-1">
                        {tr("拒绝规则", "Rejected patterns")}
                      </p>
                      <div className="space-y-1">
                        {agent.rejected_rules.length === 0 ? (
                          <p className="text-xs text-text-dim">
                            {tr("暂无拒绝规则。", "No rejected rules yet.")}
                          </p>
                        ) : (
                    ...
                    <div>
                      <p className="text-xs text-text-muted mb-1">
                        {tr("系统事件日志", "System event logs")}
                      </p>
                      <div className="space-y-1">
                        {agent.system_logs.length === 0 ? (
                          <p className="text-xs text-text-dim">
                            {tr("暂无系统日志。", "No system logs yet.")}
                          </p>
                        ) : (
                          agent.system_logs.slice(0, 6).map((log) => (
                            <div
                              key={log.id}
                              className="text-[11px] border border-border rounded-md p-2 bg-bg-input/40"
                            >
        ...
        <div className="bg-bg-card border border-border rounded-xl p-6">
          ...
          {memoryLoading ? (
            <p className="text-sm text-text-muted">
              {isZh ? "加载分身风格中..." : "Loading persona settings..."}
            </p>
          ) : memoryAgents.length === 0 ? (
            <p className="text-sm text-text-muted">
              {isZh ? "暂无可配置的 Agent" : "No agents available."}
            </p>
          ) : (
            <div className="space-y-4">
              {personaAgents.map((agent) => {
                ...
                return (
                  <div
                    ...
                    className={`border rounded-lg p-3 space-y-3 ${highlightedAgentId === agent.id ? "border-primary/50 bg-primary/5" : "border-border"}`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="text-sm font-medium">{agent.name}</h3>
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded border ${agent.persona?.mode === "live" ? "border-emerald-500/30 text-emerald-500 bg-emerald-500/10" : "border-zinc-500/30 text-zinc-400 bg-zinc-500/10"}`}
                      >
                    ...
                    {!agent.persona ? (
                      <p className="text-xs text-text-dim">
                        {isZh ? "分身数据不可用" : "Persona data unavailable."}
                      </p>
                    ) : (
                      <>
                          ...
                          <div>
                            <div className="flex items-center justify-between text-xs text-text-muted mb-1">
                              ...
                              <span className="font-medium text-text">
                                L{tier.level} ·{" "}
                                {isZh ? tier.labelZh : tier.labelEn}
                              </span>
                            </div>
                            <input
                              type="range"
                              ...
                              {modeMenuAgentId === agent.id ? (
                                <div className="absolute z-20 mt-1 w-full rounded-md border border-border bg-bg-card shadow-lg p-1">
                                  <button
                                    ...
                                    className={`w-full text-left text-xs px-2 py-1.5 rounded ${agent.persona.mode === "shadow" ? "bg-primary/10 text-primary font-medium" : "text-text hover:bg-bg-input"}`}
                                  >
                                    {isZh
                                      ? "观察学习（推荐）"
                                      : "Learning only (recommended)"}
                                  </button>
                                  <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    className={`w-full text-left text-xs px-2 py-1.5 rounded ${agent.persona.mode === "live" ? "bg-primary/10 text-primary font-medium" : "text-text hover:bg-bg-input"}`}
                                  >
                        ...
                        <div className="text-[11px] text-text-muted">
                          {isZh ? "当前风格稳定度" : "Current style stability"}:{" "}
                          {Math.round((agent.persona.confidence || 0) * 100)}%
                        </div>
                        {personaPreviewByAgent[agent.id] ? (
                          <div className="space-y-2">
                            <div className="rounded-md border border-border bg-bg-input/40 p-2">
                              ...
                              <p className="text-xs text-text whitespace-pre-wrap wrap-break-word">
                                {personaPreviewByAgent[agent.id]?.baseline ||
                            ...
                            <div className="rounded-md border border-primary/25 bg-primary/5 p-2">
                              ...
                              <p className="text-xs text-text whitespace-pre-wrap wrap-break-word">
                                {personaPreviewByAgent[agent.id]?.persona ||
                                  (isZh ? "暂无内容" : "No content")}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          {isPersonaDirty(agent.id, agent.persona) ? (
                            ...
                            <span className="text-[11px] text-text-dim">
                              {isZh
                                ? "当前无待保存风格改动"
                                : "No unsaved style changes"}
                            </span>
                          )}
                          <button
                            type="button"
                            onClick={() => {
                              setPreviewDialogAgent({
                                id: agent.id,
                                name: agent.name,
                              });
      ...
      {ruleDialog && (
        ...
        >
          <div
            className="bg-bg-card border border-border rounded-xl p-5 w-full max-w-lg mx-4"
          ...
          >
            ...
            <textarea
              ...
              className="w-full bg-bg-input border border-border rounded-md px-3 py-2 text-sm text-text focus:outline-none focus:border-primary min-h-24"
              placeholder={isZh ? "输入规则内容..." : "Enter rule text..."}
              maxLength={300}
            />
      ...
      {previewDialogAgent && (
        ...
        >
          <div
            className="bg-bg-card border border-border rounded-xl p-5 w-full max-w-2xl mx-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-base font-semibold">
                {isZh
                  ? `风格预览 · ${previewDialogAgent.name}`
                  : `Style Preview · ${previewDialogAgent.name}`}
              </h3>
              <button
                type="button"
                onClick={() => setPreviewDialogAgent(null)}
            ...
            <textarea
              ...
              className="w-full bg-bg-input border border-border rounded-md px-3 py-2 text-sm text-text focus:outline-none focus:border-primary min-h-25"
              placeholder={
                isZh
                  ? "输入想预览的场景..."
                  : "Enter scenario for style preview..."
              }
            />
            <div className="mt-4 flex justify-end gap-2">
              <button
              ...
              <button
                type="button"
                onClick={() =>
                  void handlePreviewPersona(
                    previewDialogAgent.id,
                    previewScenarioText,
                  )
                }
